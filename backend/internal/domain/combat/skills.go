...
func (s *BaseDamageSkill) Execute(attacker *Entity, target *Entity) ([]Tick, error) {
	...
	// Calculate Critical Hit (Base 15% chance for now)
	isCritical := rand.Float64() < 0.15
	critMultiplier := 1.0
	if isCritical {
		critMultiplier = 1.5
	}
	...
	finalDamage := float64(damage) * comboBonus * momentumBonus * critMultiplier
	damage = int(math.Floor(finalDamage))

	// Increment Combo and Momentum on hit
	attacker.Combo++
	attacker.Momentum += 0.5 // Buffed momentum gain
	if attacker.Momentum > 10.0 {
		attacker.Momentum = 10.0
	}
	...
	// Stickier Combos: Only break fully on Critical hits
	// Otherwise just reduce by 1 to represent staggering
	if isCritical {
		target.Combo = 0
	} else if target.Combo > 0 {
		target.Combo--
	}
