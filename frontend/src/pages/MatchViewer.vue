  ...
  <div class="pixel-theme h-dvh max-h-dvh overflow-hidden flex flex-col gap-3 sm:gap-4 font-mono text-slate-200" :style="{ backgroundImage: `url('${PIXEL_ASSETS.BG_DUNGEON}')` }">
    ...
    <div class="relative z-10 flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 min-h-0 px-3 sm:px-4 pb-3 sm:pb-4 overflow-y-auto lg:overflow-hidden">
      ...
      <div class="lg:col-span-2 flex flex-col gap-3 sm:gap-4 min-h-0 min-w-0">
        ...
        <div class="relative flex-1 min-h-[45vh] sm:min-h-[55vh] border-4 border-slate-700 bg-[#020617] overflow-hidden shadow-2xl flex flex-col group pixel-box">
          ...
          <div class="lg:hidden absolute bottom-0 inset-x-0 bg-slate-900/95 border-t-2 border-slate-700 p-2 flex flex-col gap-1">
            <div class="relative h-2 bg-slate-950 border border-slate-700 cursor-pointer" @click="seekToPercent">
            ...
            <div v-if="showConfig" class="flex items-center justify-center gap-4 pt-1 border-t border-slate-700">
              <div class="flex items-center gap-1">
                ...
                <input type="range" min="0.5" max="4" step="0.5" v-model.number="playbackSpeed" class="w-14 accent-amber-500 h-1 bg-slate-800" />
      ...
      <div class="border-4 border-slate-700 bg-slate-950 flex flex-col overflow-hidden min-h-0 shadow-xl pixel-box">
       ...
       <!-- Mobile Log Modal -->
       <div v-if="showLog" class="lg:hidden">
         <transition name="slide-up">
           <div class="fixed inset-x-0 bottom-0 z-50 h-[30svh] bg-slate-900 border-t-4 border-amber-600 flex flex-col shadow-2xl">
             ...
             <div class="flex-1 overflow-y-auto px-2 pb-2 space-y-1 custom-scrollbar">
               <div v-for="round in ticks" :key="round.round" @click="selectRound(round.round); showLog = false;" class="p-2 text-[10px] cursor-pointer border" :class="round.round === selectedRound ? 'border-amber-600 bg-amber-900/20' : 'border-slate-700 hover:border-slate-500'">
         ...
         <div @click="showLog = false" class="fixed inset-0 z-40 bg-black/50"></div>
       </div>
