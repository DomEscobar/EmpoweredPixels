  ...
  <div class="pixel-theme h-dvh max-h-dvh overflow-hidden flex flex-col gap-3 sm:gap-4 font-mono text-slate-200" :style="{ backgroundImage: `url('${PIXEL_ASSETS.BG_DUNGEON}')` }">
    ...
    <div class="relative z-10 flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 min-h-0 px-3 sm:px-4 pb-3 sm:pb-4 overflow-y-auto lg:overflow-hidden">
      ...
      <transition name="slide-up">
        <div v-if="showLog" class="lg:hidden fixed inset-x-0 bottom-0 z-50 h-[30svh] bg-slate-900 border-t-4 border-amber-600 flex flex-col">
          ...
          <div class="flex-1 overflow-y-auto px-2 pb-2 space-y-1 custom-scrollbar">
            <div v-for="round in ticks" :key="round.round" @click="selectRound(round.round); showLog = false;" class="p-2 cursor-pointer border rounded text-xs" :class="round.round === selectedRound ? 'border-amber-600 bg-amber-900/20' : 'border-slate-700 hover:border-slate-500'">
      ...
      <div v-if="showLog" @click="showLog = false" class="lg:hidden fixed inset-0 z-40 bg-black/50"></div>
