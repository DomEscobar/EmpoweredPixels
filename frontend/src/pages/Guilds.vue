<template>
  <div class="p-6">
    <h1 class="text-3xl font-bold mb-6">Guilds</h1>
    ...
    <div v-else>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="guild in guilds" :key="guild.id" class="bg-slate-900 border border-slate-800 p-6 rounded-lg">
          <h2 class="text-xl font-bold text-indigo-400">{{ guild.name }}</h2>
          <p class="text-slate-400 mt-2 text-sm h-12 overflow-hidden">{{ guild.description }}</p>
      ...
      <div v-if="guilds.length === 0" class="text-center p-12 bg-slate-900 rounded-lg border border-slate-800">
        <p class="text-slate-400">No guilds found. Be the first to start a legacy!</p>
