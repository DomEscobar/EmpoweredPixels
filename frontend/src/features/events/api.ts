...
export async function getCurrentEvents(token: string): Promise<ActiveEvent[]> {
  const response = await fetch(`${API_URL}/api/events/current`, {
...
export async function getNextEvent(token: string): Promise<NextEventInfo> {
  const response = await fetch(`${API_URL}/api/events/next`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!response.ok) throw new Error("Failed to fetch next event");
  return response.json();
}
